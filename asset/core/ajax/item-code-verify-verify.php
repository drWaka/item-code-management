  ...
  if (isset($_POST['recordId'])) {
    ...
    if ($recordId -> valid == 1) {
      ...
      $transactQry = '
        ...
        FK_verify_stat = (
          SELECT PK_status FROM status 
          WHERE stat_desc = "verified" AND type = "VF" 
        ),
        FK_verifier = "' . $_SESSION['userId'] . '",
      ...
      if ($connection -> query($transactQry)) {
        ...
        $verifierQry = '
          ...
          WHERE c.user_type_desc IN ("administrator", "encoder") AND b.isActive = 1
        ';
        ...
        $emailContent -> title = 'ICM #' . $recordId -> value . ' : New Item Code Request Verified by ' . ucfirst($_SESSION['firstName']) . ' ' .  ucfirst($_SESSION['lastName']);
        $emailContent -> mainBody = '
          <div style="width : 100%; background-color : #F0F0F0; padding-top: 20px; padding-bottom: 20px; text-align : center; font-family: Arial, sans-serif;">
            <div style="width : 80%; text-align : left; background-color : #FFF; margin : auto; padding: 15px;">
              ...
              <br><br>
              This is an automated message generated by <b>Item Code Management (ICM) System</b> to notify you that <b>' . ucfirst($_SESSION['firstName']) . ' ' .  ucfirst($_SESSION['lastName']) . '</b> has verified a new Item Code Request <b>#' . $recordId -> value . '</b>.
        ...
        $emailContent -> alternateBody = '
          Good Day!
          This is an automated message generated by ITEM CODE MANAGEMENT (ICM) SYSTEM to Notify you that ' . strtoupper($_SESSION['firstName'] . ' ' . $_SESSION['lastName']) . 'has verified a new Item Code Request.
          
          Please use go to this link: "http://192.168.3.144:8080/item-code-management/html/item-code-encode.php" to encode the Item Code Request.
