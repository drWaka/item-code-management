  ...
  if (
    ...
    if (
      ...
      if (!empty($checkedStatBox -> value)) {
        ...
        $filter .= 'AND a.FK_verify_stat IN (SELECT sb.PK_status FROM status as sb WHERE stat_desc IN ('. $stats .') AND sb.type = "VF")';
      }
      ...
      if ($recResult -> num_rows > 0) {
        ...
        while ($recRow = $recResult -> fetch_assoc()) {
          ...
          $itemVerifier = !empty($recRow['verifier']) ? $recRow['verifier'] : 'TBD';
          $content['recContent'][] = '
            <tr class="' . $classType . '">
              ...
              <td>' . ucfirst($recRow['verifStat']) . '</td>
              <td class="capitalize">' . $recRow['requestor'] . '</td>
              <td class="capitalize">' . $itemVerifier . '</td>
              <td>
                <form action="item-code-verify-items.php" method="post" class="d-inline">
